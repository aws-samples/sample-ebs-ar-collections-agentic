...
├── destroy.sh                           # Tears down all deployed resources
├── _read_config.py                      # Helper for reading deploy-config.json values
...
│   ├── code_interpreter_policy.json     # Code Interpreter IAM policy template
...
│   ├── scripts/
│   │   └── deploy_all_rest_services.sh  # Compiles + registers PL/SQL on EBS app server
│   └── sql/                             # PL/SQL packages for EBS write-back
│       ├── XX_COLLECTIONS_REST_PKG.sql
│       ├── XX_COLLECTIONS_TASK_PKG.sql
│       └── XX_ORDER_HOLDS_PKG.sql
│
├── frontend/
│   ├── src/                             # React app
│   │   ├── App.js
...
│
├── images/                              # Architecture diagrams used by README
